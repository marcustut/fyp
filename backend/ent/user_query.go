...
func (uq *UserQuery) QueryInstances() *InstanceQuery {
	query := &InstanceQuery{config: uq.config}
...
// QuerySlides chains the current query on the "slides" edge.
func (uq *UserQuery) QuerySlides() *SlideQuery {
	query := &SlideQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withInstances: uq.withInstances.Clone(),
		withSlides:    uq.withSlides.Clone(),
...
func (uq *UserQuery) WithInstances(opts ...func(*InstanceQuery)) *UserQuery {
	query := &InstanceQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withInstances = query
	return uq
}

// WithSlides tells the query-builder to eager-load the nodes that are connected to
// the "slides" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithSlides(opts ...func(*SlideQuery)) *UserQuery {
	query := &SlideQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withInstances; query != nil {
		...
		query.Where(predicate.Instance(func(s *sql.Selector) {
			s.Where(sql.InValues(user.InstancesColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.user_instances
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_instances" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_instances" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Instances = append(node.Edges.Instances, n)
		}
	}

	if query := uq.withSlides; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[ulid.ID]*User)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.Slides = []*Slide{}
		}
		query.withFKs = true
		query.Where(predicate.Slide(func(s *sql.Selector) {
			s.Where(sql.InValues(user.SlidesColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.user_slides
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_slides" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_slides" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Slides = append(node.Edges.Slides, n)
		}
	}
