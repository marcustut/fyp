...
// Delete removes a ent.Slide from the database.
func (h SlideHandler) Delete(w http.ResponseWriter, r *http.Request) {
	l := h.log.With(zap.String("method", "Delete"))
	// ID is URL parameter.
	var err error
	id := chi.URLParam(r, "id")
	err = h.client.Slide.DeleteOneID(string(id)).Exec(r.Context())
	if err != nil {
		switch {
		case ent.IsNotFound(err):
			...
			NotFound(w, msg)
		default:
			l.Error("could-not-delete-slide", zap.Error(err), zap.String("id", string(id)))
			InternalServerError(w, nil)
		}
		return
	}
	l.Info("slide deleted", zap.String("id", string(id)))
