...
func (f SlideFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.SlideMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.SlideMutation", m)
	}
	return f(ctx, mv)
}
